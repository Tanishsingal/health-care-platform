...
export function getPool(): Pool {
  if (!pool) {
    ...
    // Log pool creation only once in development
    if (process.env.NODE_ENV === 'development' && !isPoolInitialized) {
      console.log('🔗 Database connection pool initialized');
      console.log(`📊 Pool config: max=${dbConfig.max} clients, idle timeout=${dbConfig.idleTimeoutMillis}ms`);
      isPoolInitialized = true;
      
      // Optional: Log pool statistics periodically (commented out by default)
      // setInterval(() => {
...
export async function query<T = any>(
  ...
  try {
    ...
    // Log slow queries in development (threshold: 2 seconds)
    // Neon DB can be slow on cold starts, so we use a higher threshold
